...
const updateTable = async (tableName: string, otherCols: string[]) => {
    ...
    await Promise.all(
        data.map(async (row: Record<string, unknown>) => {
            ...
            const setFragments = [
                sql`created = ${c.toISOString()}`,
                sql`updated = ${u.toISOString()}`
            ];
            if (isVersionTable) {
...
export default async (...otherCols: string[]) => {
    ...
    return await Promise.all(typed.map((t) => updateTable(t.table_name, tableCols.filter(tc => tc[0] === t.table_name).map(tc => tc[1]))));
};
