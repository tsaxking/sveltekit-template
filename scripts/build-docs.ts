...
const titleFromPath = (relativePath: string) => {
	...
	const fallback = base.toLowerCase() === 'readme' ? path.basename(path.dirname(relativePath)) : base;
...
const writeSidebar = async () => {
	...
	const apiTree = buildTree(apiFiles.map((p) => p.replace(/^api\//, '')), 'api');
	const notesTree = buildTree(notesFiles.map((p) => p.replace(/^notes\//, '')), 'notes');

	const sidebar = {
		'/api/': [
			{ text: 'API', link: '/api/' },
			...toSidebarItems(apiTree)
		],
		'/notes/': [
			{ text: 'Notes', link: '/notes/' },
			...toSidebarItems(notesTree)
		]
	};
