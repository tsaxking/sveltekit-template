...
export default new Folder(
    ...
    [
        new Action(
            'Block Page',
            'Block access to a spcific page for all users',
            '🚫',
            async () => {
                try {
                    await routeTree();
                ...
                const currentlyBlockedLines = currentlyBlocked.split('\n').filter(line => line.trim() !== '');
        ...
        new Action(
            ...
            async () => {
                ...
                const currentlyLimitedLines = currentlyLimited.split('\n').filter(line => line.trim() !== '');
                const routeFile = fs.readFileSync(
                    path.join(process.cwd(), 'private', 'route-tree.pages'),
                    'utf-8'
                );
        ...
        new Folder(
            'Delete IP Page Ruleset',
            'Delete an existing IP-based page ruleset',
            '🗑️',
            [
                new Action(
                    'Delete IP',
                    'Delete all rulesets for a specific IP address',
                    '🗑️',
                    async () => {
                        const ip = await prompt({
                        ...
                        if (await confirm({
                            ...
                            await Limiting.PageRuleset.fromProperty('ip', valid.data, { type: 'stream', includeArchived: true, }).pipe(rs => rs.delete());
                            await alert({
                ...
                new Action(
                    ...
                    async () => {
                        ...
                        if (await confirm({
                            ...
                            await Limiting.PageRuleset.fromProperty('page', page, { type: 'stream', includeArchived: true, }).pipe(rs => rs.delete());
                            await alert({
                ...
                new Action(
                    ...
                    async () => {
                        ...
                        const select = await selectData(
                            rules.value,
                            'Select a ruleset to delete',
                            {
                ...
                new Action(
                    'Search Rulesets by IP',
                    'Search for rulesets by IP address',
                    '🔍',
                    async () => {
                        const ip = await prompt({
