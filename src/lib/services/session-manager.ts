import { closeManager, startManager, managerSend, getManagerConnections, redirectConnection, reloadConnection, sendToConnection, getActiveConnections } from "$lib/utils/remote-utils.remote";
...
export class SessionManager extends WritableArray<Connection> {
    ...
    public readonly state = new WritableBase<"Connecting..." | "Connected" | "Disconnected">("Disconnected");
        ...
        return attemptAsync(async () => {
            ...
            while (true) {
                try {
                    ...
                    console.error('Failed to start session manager, retrying in', delay / 1000, 'seconds:', error);
                    await sleep(delay);
...
sse.on('session-manager:connection-connect', (data) => {
    ...
    if (!parsed.success) return console.error('Invalid session-manager:connection-connect data:', parsed.error);
...
sse.on('session-manager:connection-disconnect', (data) => {
    ...
    if (!parsed.success) return console.error('Invalid session-manager:connection-disconnect data:', parsed.error);
