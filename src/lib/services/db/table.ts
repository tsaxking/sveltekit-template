import { type Subscriber, type Unsubscriber, type Updater, type Writable, writable, get } from "svelte/store";
import { _init, _define, type SchemaFieldReturnType, type SchemaDefinition, type TableStructable } from ".";
...
export class Table<Name extends string, Type extends SchemaDefinition> {
    ...
    all(config: ReadConfig<boolean>) {
        return attemptAsync(async () => {
            ...
            const w = new TableDataArr(this, datas.map((data) => this.Generator(data)));
     ...
     * @returns {ResultPromise<TableDataArr<Name, Type>>} Promise resolving to a reactive data array
     */
    fromProperty<K extends keyof Type>(key: K, value: SchemaFieldReturnType<Type[K]>, config: ReadConfig<false>): ResultPromise<TableDataArr<Name, Type>>;
     ...
     * @returns {ResultPromise<PaginatedTableData<Name, Type>>} Promise resolving to a paginated data array
     */
    fromProperty<K extends keyof Type>(key: K, value: SchemaFieldReturnType<Type[K]>, config: ReadConfig<true>): ResultPromise<PaginatedTableData<Name, Type>>;
     ...
     */
    fromProperty<K extends keyof Type>(key: K, value: SchemaFieldReturnType<Type[K]>, config: ReadConfig<boolean>) {
        return attemptAsync(async () => {
            _init();
            if (config.pagination) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const total = await this.table().where(key as string).equals(value as any).count();
                const datas = await this.table()
                    .where(key as string)
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    .equals(value as any)
                ...
                const arr = new PaginatedTableData(
                    ...
                    async () => {
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        return await this.table().where(key as string).equals(value as any).count();
                    }
                );
                const offNew = this.on('new', (d) => {
                    if (d.data[key] === value) {
            ...
            if (has) return has;
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const datas = await this.table().where(key as string).equals(value as any).toArray();
            const arr = new TableDataArr(this, datas.map((data) => this.Generator(data)));
 ...
 */
export class TableData<Name extends string, Type extends SchemaDefinition> implements Writable<TableStructable<Type>> {
    /**
 ...
 */
export class TableDataArr<Name extends string, Type extends SchemaDefinition> implements Writable<TableData<Name, Type>[]> {
    /**
 ...
 */
export class PaginatedTableData<Name extends string, Type extends SchemaDefinition> extends TableDataArr<Name, Type> {
    /**
