...
export const managerSend = command(z.object({
    manager: z.string(),
    event: z.string(),
    data: z.unknown(),
}), async (data) => {
...
export const getManagerConnections = query(z.string(), async (managerId) => {
    ...
    const manager = SessionManager.managers.get(managerId);
    if (!manager) throw new Error('Session manager not found: ' + managerId);
    return manager.getConnections().map(c => c.connections.map(con => ({
        id: con.uuid,
...
export const redirectConnection = command(z.object({
    id: z.string(),
    url: z.string(),
...
export const reloadConnection = command(z.object({
    id: z.string(),
    reason: z.string().optional(),
}), async ({ id, reason }) => {
...
export const sendToConnection = command(z.object({
    id: z.string(),
    event: z.string(),
    data: z.unknown(),
}), async ({ id, event, data }) => {
