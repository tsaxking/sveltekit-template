...
export namespace Limiting {
    ...
    export const isBlocked = (session: Session.SessionData, account?: Account.AccountData) => {
        return attemptAsync<{
            ...
            const ipBlocked = await BlockedIps.fromProperty('ip', session.data.ip, { type: 'single' }).unwrap();
            if (ipBlocked) return { blocked: true, reason: ipBlocked.data.reason };
            const sessionBlocked = await BlockedSessions.fromProperty('session', session.id, { type: 'single' }).unwrap();
            if (sessionBlocked) return { blocked: true, reason: sessionBlocked.data.reason };
            const fingerprintBlocked = await BlockedFingerprints.fromProperty('fingerprint', session.data.fingerprint, { type: 'single' }).unwrap();
            if (fingerprintBlocked) return { blocked: true, reason: fingerprintBlocked.data.reason };
            if (account) {
                const accountBlocked = await BlockedAccounts.fromProperty('account', account.id, { type: 'single' }).unwrap();
    ...
    export const violate = async (session: Session.SessionData, account: Account.AccountData | undefined, increment: number) => {
        return attemptAsync(async () => {
            ...
            const score = Math.max(...await Promise.all([
                ...
                account ? Redis.getValue(`violation_account:${account.id}`, z.number()) : Promise.resolve('0'),
            ...
            const sessionExists = await BlockedSessions.fromProperty('session', session.id, { type: 'single' }).unwrap();
            if (!sessionExists) {
                await BlockedSessions.new({
                    session: session.id,
                    reason: `Rate limit violation with score ${score}`,
                }).unwrap();
            } else {
                await sessionExists.update({
                    reason: `Rate limit violation with score ${score}`,
                }).unwrap();
            }

            const ipExists = await BlockedIps.fromProperty('ip', session.data.ip, { type: 'single' }).unwrap();
            if (!ipExists) {
                ...
                await ipExists.update({
                    reason: `Rate limit violation with score ${score}`,
                }).unwrap();
            }

            const fingerprintExists = await BlockedFingerprints.fromProperty('fingerprint', session.data.fingerprint, { type: 'single' }).unwrap();
            if (!fingerprintExists) {
                ...
                await fingerprintExists.update({
                    reason: `Rate limit violation with score ${score}`,
                }).unwrap();
            }
    ...
    export const violationSeverity = (session: Session.SessionData, account?: Account.AccountData) => {
        return attemptAsync(async () => {
            const ipScore = await Redis.getValue(`violation_ip:${session.data.ip}`, z.number()).then(res => Number(res) || 0);
            const sessionScore = await Redis.getValue(`violation_session:${session.id}`, z.number()).then(res => Number(res) || 0);
            const fingerprintScore = await Redis.getValue(`violation_fingerprint:${session.data.fingerprint}`, z.number()).then(res => Number(res) || 0);
            const accountScore = account ? await Redis.getValue(`violation_account:${account.id}`, z.number()).then(res => Number(res) || 0) : 0;
