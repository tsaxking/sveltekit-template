...
import { globalCols, StructStream, type Blank, type Struct, type Structable, type StructData, type TsType } from "drizzle-struct/back-end";
import { z } from "zod";
 ...
 */
type CB<DataType, ReturnType> = (request: RequestEvent, data: DataType) => ListenerStatus<ReturnType> | Promise<ListenerStatus<ReturnType>>;
...
class ListenerBase<Event extends string, DataType, ReturnType> {
     ...
     */
    async run(request: RequestEvent): Promise<ListenerStatus<ReturnType>> {
        let obj: unknown;
        try {
            obj = await request.request.json();
        } catch (error) {
 ...
 */
export class CallListener<Event extends string, DataType, ReturnType extends CallReturnType> extends ListenerBase<Event, DataType, ReturnType> {
    /**
     ...
     */
    public static on<Event extends string, DataType, ReturnType extends CallReturnType>(event: Event, struct: Struct<any, any>, schema: z.ZodType<DataType>, cb: CB<DataType, ReturnType>) {
 ...
 */
type QueryReturnType<StructType extends Blank, Name extends string> = StructData<StructType, Name>[] | Promise<StructData<StructType, Name>[]>;


/**
 * Callback type for query listeners.
 */
type QueryCBReturnType<StructType extends Blank, Name extends string> = StructData<StructType, Name>[] | StructStream<StructType, Name>;
/**
 * Type for query callback function.
 */
type QueryCBType<DataType, StructType extends Blank, Name extends string> = (request: RequestEvent, data: DataType) => QueryCBReturnType<StructType, Name> | Promise<QueryCBReturnType<StructType, Name>>;
 ...
 */
export class QueryListener<Event extends string, StructType extends Blank, Name extends string, DataType, ReturnType extends QueryReturnType<StructType, Name>> extends ListenerBase<Event, DataType, ReturnType> {
    /**
     * Map of all registered query listeners by event name.
     */
    public static readonly listeners = new Map<string, QueryListener<string, Blank, string, any, any>>();

    /**
     * Register a new query listener for an event.
     */
    public static on<Event extends string, StructType extends Blank, Name extends string, DataType>(event: Event, struct: Struct<StructType, Name>, schema: z.ZodType<DataType>, cb: QueryCBType<DataType, StructType, Name>) {
        if (QueryListener.listeners.has(event)) {
            throw new Error('Listener already exists for event: ' + event);
        }
        const listener = new QueryListener(event, struct, async (request, event) => {
 ...
 */
export class SendListener<Event extends string, DataType, ReturnType extends SendReturnType> extends ListenerBase<Event, DataType, ReturnType> {
    /**
     ...
     */
    public static on<Event extends string, DataType, ReturnType extends SendReturnType>(event: Event, struct: Struct<any, any>, schema: z.ZodType<DataType>, cb: CB<DataType, ReturnType>) {
...
type Read<_S extends Blank, Type extends ReadTypes> = 
    Type extends 'all' | 'archived' ? void
        : Type extends 'property' ? {
            key: keyof _S,
            value: TsType<_S[keyof _S]['_']['dataType']>,
        }
        : Type extends 'from-id' ? IDOnly : never;
...
type ActionDataType<Action extends Actions, _S extends Blank, ReadType extends ReadTypes> = 
    Action extends DataAction.Archive ? IDOnly
        : Action extends DataAction.Create ? Create<Blank>
        : Action extends DataAction.Delete ? IDOnly
        : Action extends DataAction.DeleteVersion ? VersionAction
        : Action extends DataAction.RestoreArchive ? IDOnly
        : Action extends DataAction.RestoreVersion ? VersionAction
        : Action extends PropertyAction.Read ? Read<_S, ReadType>
        : Action extends PropertyAction.ReadArchive ? Read<_S, 'archived'>
        : Action extends PropertyAction.ReadVersionHistory ? Read<_S, 'from-id'>
        : Action extends PropertyAction.Update ? Update<_S>
        : Action extends 'set-attributes' ? Attributes
        : never;
type BypassDataConfig<Action extends Actions, _S extends Blank, ReadType extends ReadTypes = ReadTypes> = unknown;
type BypassFn<Action extends Actions, _S extends Blank, R extends ReadTypes> = (request: RequestEvent, data: BypassDataConfig<Action, _S>) => boolean | Promise<boolean>;
type BypassException<Action extends Actions, _S extends Blank, R extends ReadTypes> = (request: RequestEvent, data: BypassDataConfig<Action, _S>) => boolean | Promise<boolean>;
