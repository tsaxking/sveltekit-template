...
export class SessionManager {
    ...
    init() {
        ...
        offs.push(this.on('connect', (connection) => {
            if (connection.sessionId === this.owner.sessionId) return;
            this.owner.send('session-manager:connection-connect', { id: connection.uuid, sessionId: connection.sessionId, url: connection.url, generalizedUrl: connection.generalizedUrl });
        }));

        offs.push(this.on('disconnect', (connection) => {
            if (connection.sessionId === this.owner.sessionId) return;
            this.owner.send('session-manager:connection-disconnect', { id: connection.uuid, sessionId: connection.sessionId });
        }));

        const onclose = () => {
            console.log('SessionManager owner disconnected, deleting manager:', this.id);
            this.delete();
...
sse.on('connect', (connection) => {
    ...
    for (const m of SessionManager.managers.values()) {
        ...
        m.owner.send('session-manager:connection-connect', { id: connection.uuid, sessionId: connection.sessionId, url: connection.url, generalizedUrl: connection.generalizedUrl });
    }
});
...
sse.on('disconnect', (connection) => {
    ...
    for (const m of SessionManager.managers.values()) {
        ...
        m.owner.send('session-manager:connection-disconnect', { id: connection.uuid, sessionId: connection.sessionId });
    }
});
