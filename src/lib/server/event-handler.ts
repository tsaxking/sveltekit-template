...
// 						if (!safe.success) return error(new DataError(struct, 'Invalid Read property'));
// 						data = await struct
// 							.fromProperty(safe.data.key, safe.data.value as any, {
// 								type: 'all'
// 							})
// 							.unwrap();
// 					}
...
// 						if (!safe.success) return error(new DataError(struct, 'Invalid Read universe'));
// 						data = await struct
// 							.fromProperty('universe', safe.data.universe, {
// 								type: 'all'
// 							})
// 							.unwrap();
// 					}
...
export const Errors = {
	...
	notPermitted: (
		...
		return new Response(
			JSON.stringify({
				success: false,
				message: `Account ${account.id} is not permitted to perform action ${action} on struct ${structName}`
			}),
