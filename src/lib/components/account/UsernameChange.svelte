<script lang="ts">
    ...
    const test = () => {
        ...
        testTimeout = setTimeout(() => {
            Account.usernameExists(username).unwrap().then((result) => {
                if (result) {
...
<div class="form-floating mb-3">
    <input type="text" class="form-control" id="change-username" placeholder="Change Username" bind:value={username} oninput={test}>
