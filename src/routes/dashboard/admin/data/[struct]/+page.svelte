        ...
        <Grid 
            ...
            opts={{
                columnDefs: [
                    ...
                    ...Object.entries(structType).filter(([k]) => !safes?.includes(k)).map(([key, value]) => ({
                        field: 'data.' + key as any,
                        headerName: capitalize(fromCamelCase(key)),
                        valueGetter: [
                            'created',
                            'updated',
                        ].includes(key) ? (params: ValueGetterParams<StructData<Blank>>) => dateTime(new Date(String(params.data?.data[key]))) : 
                        key === 'attributes' ? (params: ValueGetterParams<StructData<Blank>>) => (JSON.parse(params.data?.data.attributes || '[]') as string[]).join(', ') : undefined,
                        ...
                        valueSetter: (params: ValueSetterParams<StructData<Blank>>) => {
                            ...
                            cs.execute({
                                ...
                                do: () => {
                                    if (key === 'attributes') {
                                        params.data.setAttributes(String(params.newValue).split(',').map(a => a.trim()))
                                        return;
                                    }
                                ...
                                undo: () => {
                                    if (key === 'attributes') {
                                        params.data.setAttributes(String(params.oldValue).split(',').map(a => a.trim()))
                                        return; 
                                    }
            ...
            {#each Object.entries(structType).filter(([k]) => !globalCols.includes(k)) as [key, value]}
                <div class="row mb-3">
                    ...
                    <div class="form-floating">
                        ...
                        />
                        <label class="ms-3">{capitalize(fromCamelCase(key))}</label>
                    </div>
