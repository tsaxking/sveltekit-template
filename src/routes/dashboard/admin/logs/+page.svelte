...
<div class="container">
    <div class="row mb-3">
        ...
        <div class="col">
            ...
            <option value=""
                selected={struct === undefined}
            >All</option>
            {#each structs as s}
                <option value={s}
                    selected={s === struct}
                >{capitalize(fromSnakeCase(s))}</option>
            {/each}
        </select>
        </div>
    </div>
                ...
                <tbody>
                    {#each logs as log}
                        <tr>
                            ...
                            <td title={log.accountId} class="cursor-pointer" onclick={() => copy(log.accountId, true)}>{log.account?.username || log.accountId}</td>
                            <td title={log.type} class="cursor-pointer" onclick={() => copy(log.type, true)}>{log.type}</td>
                            <td title={log.dataId} class="cursor-pointer" onclick={() => copy(log.dataId, true)}>{abbreviate(log.dataId)}</td>
                            <td title={log.struct} class="cursor-pointer" onclick={() => copy(log.struct, true)}>{capitalize(fromSnakeCase(log.struct))}</td>
                            <td title={log.message} class="cursor-pointer" onclick={() => copy(log.message, true)}>{log.message}</td>
                        </tr>
                    {/each}
                </tbody>
            </table>
