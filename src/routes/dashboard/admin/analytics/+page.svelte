    ...
    const render = () => {
        ...
        chart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Page Views',
                    data: Object.keys(pages).filter(p => $items.includes(p)).map(page => pages[page].views),
                }, {
                    label: 'Averate Retention (minutes)',
                    data: Object.keys(pages).filter(p => $items.includes(p)).map(page => (pages[page].retention / pages[page].views) / 60),
                }, {
                    label: 'Unique Visitors',
                    data: Object.keys(pages).filter(p => $items.includes(p)).map(page => pages[page].uniqueVisitors),
                }]
            },
            options: {
                responsive: true,
                ...
                <ul class="list-group list-group-flush">
                    {#each Object.keys(pages) as item}
                        <li class="list-group-item d-flex align-items-centent ws-nowrap">
                            <input class="form-check-input me-1" type="checkbox" id="check-{item}" checked={true} onchange={() => {
