...
export const POST = async (event) => {
	// console.log('Restore version request for struct:', event.params.struct);
	...
	PERMIT: if (event.locals.account && event.params.struct !== 'test') {
		...
		{
			const blocks = struct.blocks.get(DataAction.RestoreVersion);
			if (blocks) {
				for (const block of blocks) {
					...
					if (blocked) {
						return Errors.blocked(struct.name, DataAction.RestoreVersion, block.reason);
					}
				}
			}
		}
	...
	Logs.log({
		...
		message: `${event.locals.account?.data.username || 'unknown'} restored version with vhId ${version.vhId} for data with id ${data.value.id}`
	});
